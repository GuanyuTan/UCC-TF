...
def grad(model, inputs, labels):
    with tf.GradientTape() as tape:
        ...
        return {'ucc_acc': round(acc.numpy(), 5), 'ucc_loss': round(ucc_loss.numpy(), 5), 'ae_loss':round(ae_loss.numpy(), 5), 'weighted_loss':round(loss_value.numpy(),5)}, tape.gradient(loss_value, model.trainable_variables)                                          
...
def eval(model, dataset):
    ...
    labels = tf.argmax(labels, axis=1)
    acc = accuracy(output, labels)
    return {'ucc_acc': round(acc.numpy(), 5), 'ucc_loss':  round(ucc_loss.numpy(), 5), 'ae_loss':round(ae_loss.numpy(), 5), 'weighted_loss':round(loss_value.numpy(),5)}
