...
class UCCDRNModel(tf.keras.Model):
    def __init__(self, cfg):
        ...
        if model_cfg.drn:
            ...
            for i in range(drn_num_layers+1):
                layers.append(
                    DRNLayer(
                        n_lower=self.num_features if i == 0 else 1,
                        n_upper=1,
                        q_lower=self.num_nodes if i == 0 else model_cfg.drn.hidden_q,
                        q_upper=model_cfg.drn.output_bins if i == drn_num_layers else model_cfg.drn.hidden_q,
                    )
                )
            self.drn = tf.keras.Sequential(
                [
                    tf.keras.Input((self.num_features, self.num_nodes)),
                    *layers,
                    tf.keras.layers.Reshape([model_cfg.drn.output_bins,])
                ],
                name="drn"
            )
        ...
        if model_cfg.ucc_classifier != "None":
            ...
            self.ucc_classifier = tf.keras.Sequential(
                [
                    tf.keras.Input((model_cfg.drn.output_bins, )),
    ...
    def ucc_model(self, inputs):
        ...
        feature_distributions = self.kde(
            feature_concatenated, self.num_nodes, self.sigma)
        drn_output = self.drn(feature_distributions)
        output = self.ucc_classifier(drn_output)
        return output
