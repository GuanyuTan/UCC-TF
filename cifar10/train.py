...
def grad(model, inputs, labels):
    with tf.GradientTape() as tape:
        ...
        loss_value =  model.alpha*ucc_loss + (1-model.alpha)*ae_loss + regularization_loss
        accuracy = tf.keras.metrics.Accuracy()
        output = tf.argmax(output, axis=1)
        labels = tf.argmax(labels, axis=1)
        acc = accuracy(output, labels)
        return {'ucc_acc': acc, 'ucc_loss': ucc_loss, 'ae_loss':ae_loss, 'weighted_loss':loss_value, 'regularization_loss': regularization_loss}, tape.gradient(loss_value, model.trainable_variables)                                          
        ...
        with tf.device('/device:GPU:0'):
            for i in tqdm(range(num_steps)):
                ...
                if i%10 == 0:
                    ...
                    logger.info(f"Step {i+1}, training: ucc_acc={loss_dict['ucc_acc']} weighted_loss={loss_dict['weighted_loss']} ucc_loss={loss_dict['ucc_loss']} ae_loss={loss_dict['ae_loss']} regularization_loss={loss_dict['regularization_loss']} eval: ucc_acc={eval_loss_dict['ucc_acc']} weighted_loss={eval_loss_dict['weighted_loss']} ucc_loss={eval_loss_dict['ucc_loss']} ae_loss={eval_loss_dict['ae_loss']}")
